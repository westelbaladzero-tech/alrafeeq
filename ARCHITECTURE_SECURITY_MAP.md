...
## 18.7 الإصلاح المرحلي الذي تم تنفيذه

تم تنفيذ **مرحلة آمنة أولى** بدل الربط التلقائي المباشر بين المحورين:

### أ) في `api/chat`
- إذا كان الشخص موجودًا كصديق موثّق في `friendships`
- يتم **استبعاده** من قسم `transactions.person`
- ويُعتمد فقط على قسم `═══ حسابات الأصدقاء ═══`

### ب) في `PeopleView`
- تم تغيير اللغة من لغة توحي بديون موثّقة إلى لغة أوضح:
  - `الحسابات` → `سجل الأشخاص`
  - `لـك/عليك` → `سجّلت له/سجّلت عليك`
  - `ردّ لك/أخذ منك` → `استلمت منه/أعطيته`
- وتمت إضافة تنبيه واضح أن هذا التبويب يعرض **سجل المحادثة** لا **الرصيد الموثّق**

### النتيجة
- الشات لم يعد يرى رقمين متناقضين لنفس الشخص
- المستخدم صار يميّز بصريًا بين:
  - سجل المحادثة
  - الرصيد الموثّق في تبويب الأصدقاء

### ما لم يتم بعد
- لم يتم بعد ربط `transactions.person` تلقائيًا بـ `friendships`
- ولم يتم توحيد `PeopleView` ليعتمد على محور الأصدقاء
- هذا مؤجل كقرار بنيوي لاحق لأنه أعلى خطورة من الإصلاح المرحلي
