...
# 18) نقطة التقاطع الحرجة بين محور المحادثة ومحور الأصدقاء

## 18.1 وصف المشكلة
يوجد تضارب حقيقي في **أرصدة الأشخاص** بين المحورين.

### مثال عملي
لو كتب المستخدم في الشات:
- "محمد أخذ 500"

فإن الذي يحدث الآن:
1. يتم إنشاء `transaction` في محور المحادثة مع `person = محمد`
2. يظهر محمد في `PeopleView` على أنه:
   - لك 500 جنيه
3. لكن **لا يتم إنشاء `debt_request`** في محور الأصدقاء
4. لذلك إذا فتح المستخدم `FriendsView` لنفس الشخص، قد يرى:
   - الحساب مسوّى / صفر

### النتيجة
المستخدم قد يرى **رقمين مختلفين لنفس الشخص** داخل نفس التطبيق.
...
ويعرض الأشخاص عبر:
- `FriendsView`
- قسم "حسابات الأصدقاء" في `api/chat`

### أين يحدث التعارض
- `PeopleView` لا يقرأ `debt_requests` ولا `settlements`
- `FriendsView` لا يقرأ `transactions`
- `ChatView` يقرأ **الاثنين معًا** ويضمّنهما في السياق

---

## 18.3 الخطر الناتج
هذا ليس ثغرة أمنية، لكنه **فجوة وظيفية عالية التأثير**:
- يهز ثقة المستخدم في الرصيد
- قد يجعل الشات يقدّم إجابات متناقضة
- يخلق مصدرين مختلفين للحقيقة حول نفس الشخص

---

## 18.4 القرار المعماري الحالي
### الحقيقة المعتمدة للعلاقات المالية الموثقة بين الأشخاص:
هي **محور الأصدقاء** فقط، أي:
- `friendships`
- `debt_requests`
- `settlements`
- `p2p_transactions`
...
### أما `transactions.person`:
فهو حاليًا **إشارة محادثة/تصنيف** داخل محور المحادثة، وليس مرجعًا موثّقًا للدين بين طرفين.

---

## 18.5 التوصية التنفيذية المرحلية
### المرحلة الآمنة أولًا
- لا نعتبر `PeopleView` مرجعًا نهائيًا للديون بين الأشخاص
- ونخفف لغة العرض فيه لاحقًا إذا لزم حتى لا يوهم أنه مطابق لمحور الأصدقاء

### المرحلة البنيوية اللاحقة
واحد من اتجاهين:
1. **توحيد العرض**: جعل واجهات الأشخاص/الأرصدة تقرأ من محور الأصدقاء عند الحديث عن "لك/عليك"
2. **الربط التلقائي**: أي معاملة محادثة فيها `person` تتحول أو ترتبط بمنطق `debt_requests` عندما تكون علاقة مالية موثقة

### التوصية الحالية
نبدأ مستقبلًا بالاتجاه **الأقل خطورة**:
- **توحيد العرض أولًا**
- ثم نناقش لاحقًا هل نربط `transactions.person` تلقائيًا بمحور الأصدقاء أم لا

---

## 18.6 أثر هذه النقطة على الشات
لأن `api/chat` يقرأ المصدرين معًا، يجب الانتباه إلى أن:
- إجابات الشات عن "كم لي عند فلان؟"
- أو "حسابات الناس"

قد تتأثر لو بقي المصدران متوازيين بدون توحيد.

لذلك أي تحسين لاحق في `api/chat` يجب أن يحسم:
- متى يعتمد على محور المحادثة
- ومتى يعتمد على محور الأصدقاء
