...
# 13) الفرق بين الطبقة المحلية القديمة والطبقة السحابية الأحدث

## 13.1 الطبقة المحلية القديمة
لا تزال بعض الواجهات تعتمد على `lib/store` وبيانات محلية أو شبه محلية، مثل:
- `PeopleView`
- `HistoryView`
- `DashboardView`
...
### ملاحظة مهمة
هذا يدل أن المشروع فيه **طبقتان معماريتان متداخلتان**:
1. طبقة أقدم حول `transactions` الفردية المحلية/السحابية
2. طبقة أحدث حول:
   - `debt_requests`
   - `settlements`
   - `p2p_transactions`
...
وهذا مهم جدًا لأنه يعني أن الصورة العامة ليست موحّدة بالكامل بعد، حتى لو الطبقات الحساسة المالية الجديدة صارت مقفولة.

## 13.2 الطبقة السحابية الأحدث
تركّز أكثر في:
- `FriendsView`
- `ChatView`
- مسارات `/api/*`
- العلاقات بين الأصدقاء والديون والتسويات وP2P

### النتيجة المعمارية
الجزء المالي الحديث أقوى وأكثر انضباطًا من الطبقة الأقدم، لكن ما زالت هناك حاجة لاحقًا لتوحيد المصدر الواحد للحقيقة بين:
- `transactions`
- المنظومة الحديثة متعددة الجداول
...
## 15.1 `PeopleView`
- يعرض الأشخاص من طبقة `transactions`
- يحسب صافي العلاقة من:
  - `expense` = أعطيت الشخص
  - `income` = رجع الشخص لك
- هذه طبقة عرض قديمة نسبياً مقارنةً بطبقة `FriendsView`

## 15.2 `HistoryView`
- يعتمد على `lib/store`
- يدعم حذف المعاملات من التاريخ
- يصنّف حسب الفئات والوسيلة والوقت
- هذه الواجهة تمثل سجل المعاملات العامة، لا سجل الديون/التسويات الحديثة فقط

## 15.3 `DashboardView`
- يعتمد على `getStats()`
- ملخص أعلى وأبسط
- لا يبدو أنه المصدر الأحدث للواقع المالي متعدد العلاقات، بل طبقة summary من سجل المعاملات

### ملاحظة مهمة
هذا يعزز الملاحظة المعمارية السابقة:
> التطبيق ليس طبقة واحدة فقط، بل فيه طبقة معاملات عامة، وطبقة مالية علاقية أحدث.
...
2. **الواجهة قد تعرض أكثر من نموذج بيانات في الوقت نفسه**
3. **طبقة `FriendsView` هي المركز الحالي للمنظومة المالية الأحدث**
4. **طبقة `transactions` ما زالت مؤثرة على أجزاء من التطبيق**
5. **أي إصلاح لاحق يجب أن يراعي هل هو يصلح الطبقة الحديثة فقط أم الطبقتين معًا**

---

# 17) جلسات مراجعة لاحقة مقترحة

## 17.1 جلسة توحيد المعمارية
- هل `transactions` ستظل جزءًا أساسيًا طويل المدى؟
- أم سيتم نقل الواجهات القديمة تدريجيًا إلى الطبقة الحديثة؟

## 17.2 جلسة مراجعة `lib/store`
- لأن `PeopleView` و`HistoryView` و`DashboardView` تعتمد عليه
- وهو مؤثر على دقة ما يراه المستخدم خارج `FriendsView`
