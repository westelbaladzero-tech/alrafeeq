...
export default function DashboardView() {
  ...
  return (
    <div className="p-4 space-y-4 overflow-auto h-full">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 text-green-600 mb-2">
            <TrendingUp size={18} /><span className="text-sm">وارد</span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 text-red-500 mb-2">
            <TrendingDown size={18} /><span className="text-sm">مصروف</span>
          </div>
      ...
      {stats.topCategories.length > 0 && (
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 mb-4">
            <BarChart3 size={18} className="text-[var(--accent)]" /><h3 className="font-bold">أعلى الفئات صرفاً</h3>
          </div>
          <div className="space-y-3">
            {stats.topCategories.map(([cat, amt]) => (
              <div key={cat}>
                <div className="flex justify-between text-sm mb-1">
                  <span>{cat}</span><span className="text-gray-500">{amt.toLocaleString('ar-EG')}</span>
                </div>
      ...
      {stats.count === 0 && (
        <div className="text-center text-gray-400 py-8">لا توجد عمليات بعد. ابدأ بمحادثة الرفيق لتسجيل أول مصروف أو دخل.</div>
      )}
    </div>
  );
}
